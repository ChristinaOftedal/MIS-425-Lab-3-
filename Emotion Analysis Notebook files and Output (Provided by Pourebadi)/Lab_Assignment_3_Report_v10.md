...
![](report_images_v9/figure1_model1_confusion_matrix_v9.png)

*Figure 1. Model 1 (ehcalabres) — predicted vs. actual emotion,
confusion matrix over all 240 clips.*

![](report_images_v9/figure2_model2_confusion_matrix_v9.png)
...
![](report_images_v9/figure3_confidence_comparison_v9.png)

*Figure 3. Confidence score distributions for both models, relative to
the 1/8 chance level.*

![](report_images_v9/figure4_per_emotion_accuracy_v9.png)
...
**D.1 ravdess\_summary\_report\_Model\_1.txt (ehcalabres, 180-clip
run)**
...
This is a different result from the 'sad' class discussed in Section 6.3
on the main 240-clip run — and the two should not be read as the same
finding restated. Sad's recall there (62.5%) came with weak precision
(\~25%), meaning Model 1 was over-predicting 'sad' rather than genuinely
recognizing it; the same pattern holds on this 180-clip set, where sad
recall is only 8.3% and precision just 4.9%, nowhere close to a
strength. FEARFUL here is the opposite pattern: recall 95.8% AND
precision 67.6%, so the model isn't just guessing 'fearful' often — it's
specifically right about it. In short, Model 1's single genuine bright
spot differs by dataset (sad's apparent strength in the 240-clip run
doesn't replicate here, and fearful's strength here isn't present in the
same way in the 240-clip run), which is itself worth flagging: it
suggests whatever narrow signal this model retains is inconsistent
across samples rather than a stable, class-specific competency, and the
actor composition differs between the two runs (the 240-clip set
includes Actor\_07, which this 180-clip set does not).
