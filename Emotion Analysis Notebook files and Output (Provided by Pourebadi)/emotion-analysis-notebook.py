...
#
# FIX: this script and emotion-analysis-notebook2.py both used to point at the exact same
# hardcoded path ("/workspaces/MIS-425-Lab-3-/emotion_results.csv"), so whichever script
# ran second silently overwrote the input the other one needed -- this is why the heatmap
# this script produced didn't jibe with evaluate_results.py's confusion matrix for Model 1.
# Pointing this script at its own dedicated CSV (emotion_results_Model_1.csv, which already
# has a real "True Emotion" column) fixes that.
...
# FIX: point at this model's own results file instead of the shared generic path.
path_option_1 = Path("emotion_results_Model_1.csv")
path_option_2 = Path("results.csv")
...
# FIX: this file's predicted column is called "Predicted Emotion", not "Emotion".
elif 'emotion' not in df.columns and 'Predicted Emotion' in df.columns:
    df['emotion'] = df['Predicted Emotion']
...
# FIX: this file already ships a real "True Emotion" ground-truth column (it's not just
# derived from the filename with possibly-different spelling, e.g. "fear" vs "fearful").
# Use it directly instead of re-deriving and risking a spelling mismatch with 'emotion'.
if 'True Emotion' in df.columns:
    df['actual_emotion'] = df['True Emotion']
else:
...
image_out_path = Path("prediction_patterns_heatmap.png")
...
# =====================================================================
report_path = Path("ravdess_summary_report_Model_1.txt")

try:
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("==================================================\n")
        f.write ("ehcalabres/wav2vec2-lg-xlsr-en-speech-emotion-recognition\n")
        f.write("                        Model 1\n")
